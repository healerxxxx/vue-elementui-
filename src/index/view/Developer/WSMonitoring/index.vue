...
<script>
import { DeveloperWSMonitoring } from '../../../util/api'
export default {
  ...
  methods: {
    ...
    getAccessList () {
      this.timerNum++
      this.$service(DeveloperWSMonitoring[0].accessList).then(res => {
    ...
    getStatusByAccessId (data) {
      this.$service(DeveloperWSMonitoring[0].statusByAccessId, {
        urlParams: {
    ...
    getChannelList (data, id) {
      this.$service(DeveloperWSMonitoring[0].channelListByAccessId, {
        urlParams: {
          accessId: id
        },
        params: {
    ...
    delChannel (list, data, id) {
      this.$confirm(`确定删除channel-${data.channelName}？`, {
        ...
        this.$service(DeveloperWSMonitoring[0].channelDelByAccessId, {
          urlParams: {
            accessId: id
          },
          params: {
    ...
    sidSearch () {
      ...
      this.$service(DeveloperWSMonitoring[0].clientQueryById, {
        urlParams: {
    ...
    sidDel () {
      ...
      this.$confirm(`确定删除sid-${this.sidTemp}？`, {
        ...
        if (this.sidLoading) return
        this.sidLoading = true
        this.$service(DeveloperWSMonitoring[0].clientDelById, {
          urlParams: {
            accessId: this.sidByAccessId
          },
          params: {
    ...
    getMsgDispatchCount (data) {
      this.$service(DeveloperWSMonitoring[0].msgDispatch, {
        urlParams: {
          accessId: data.accessId
        }
      }).then(res => {
    ...
    getMsgPubCount (data) {
      this.$service(DeveloperWSMonitoring[0].msgPub, {
        urlParams: {
          accessId: data.accessId
        }
      }).then(res => {

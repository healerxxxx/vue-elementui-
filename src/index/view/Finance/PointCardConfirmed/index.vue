<template>
  <div class="finance-monthlyOfSp">
    ...
    <el-form :inline="true" size="mini" :disabled="loading">
      ...
      <el-form-item>
        当月消耗总计：<b>{{pointCardAddTotal || 0}}{{filter.orderType==='1'?'豆':filter.orderType==='2'?'元':''}}</b>
        当月可确认消耗：<b>{{pointConfirmedTotal || 0}}{{filter.orderType==='1'?'豆':filter.orderType==='2'?'元':''}}</b>
      </el-form-item>
    </el-form>
    <el-table ref="elTable" :data="list" stripe v-loading="loading" style="width: 100%"
      ...
      <el-table-column prop="monthserviceDays" label="当月服务天数" min-width="108">
        <template slot-scope="scope">
          {{scope.row.monthserviceDays}}天
        </template>
      </el-table-column>
      <el-table-column prop="confirmedPoint" label="当月可确认消耗点数" min-width="100">
      </el-table-column>
      <el-table-column prop="confirmedPrice" label="当月可确认消耗" min-width="120">
        <template slot-scope="scope">
          {{scope.row.confirmedPrice}}{{filter.orderType==='1'?'豆':'元'}}
        </template>
      </el-table-column>
...
<script>
import { financePointCardConfirmed } from '../../../util/api'
...
export default {
  ...
  methods: {
    ...
    getDiscountList () {
      this.$service(financePointCardConfirmed.discountList, {
        method: 'get'
      }).then(res => {
        if (res.code === 200) {
    ...
    search () {
      this.getList()
      this.getPointCardAddTotal()
      this.getPointConfirmedTotal()
    },
    getList (page) {
      if (this.loading) return
      this.loading = true
      this.pageIndex = page || 1
      this.$service(financePointCardConfirmed.list, {
        method: 'post',
        data: Object.assign({
    ...
    getPointCardAddTotal () {
      this.$service(financePointCardConfirmed.sum, {
        method: 'post',
    ...
    getPointConfirmedTotal () {
      this.$service(financePointCardConfirmed.addTotal, {
        method: 'post',
        data: this.resetFilter()
      }).then(res => {
        if (res.code === 200) {
          this.pointConfirmedTotal = res.data
        }
      })
    },
    exportList () {
      ...
      window.open(location.origin + financePointCardConfirmed.export + `?${_arr.join('&')}`)
    }
  },
  watch: {
    windowHeight () {

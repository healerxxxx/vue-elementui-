<template>
  <div class="finance-monthlyOfSp">
    ...
    <el-form :inline="true" size="mini" :disabled="loading">
      ...
      <el-form-item>
        <span>当月总计新增金额：<b>{{total || 0}}</b>；</span>
        <span v-if="hiddenColumn">当月总计赠送积分：<b>{{sumMonthCredit || 0}}</b>；</span>
        <span v-if="hiddenColumn">累积赠送积分总数：<b>{{sumCredit || 0}}</b></span>
      </el-form-item>
    </el-form>
    <el-table
      ref="elTable"
...
<script>
import { financeMonthlyOfSp } from '../../../util/api'
...
export default {
  ...
  computed: {
    ...
    financeMonthlyOfSpApi () {
      return financeMonthlyOfSp[this.appTab]
    },
    hiddenColumn () {
  ...
  methods: {
    ...
    getList (page) {
      ...
      this.$service(this.financeMonthlyOfSpApi.list, {
        method: 'get',
        ...
        res => {
          ...
          if (res.code === 200) {
            ...
            this.sumCredit = res.data.sumCredit
            this.sumMonthCredit = res.data.sumMonthCredit
          } else {
    ...
    exportList () {
      ...
      window.open(location.origin + this.financeMonthlyOfSpApi.export + '?' + _arr.join('&'))
    }
  },
  watch: {
    windowHeight () {
